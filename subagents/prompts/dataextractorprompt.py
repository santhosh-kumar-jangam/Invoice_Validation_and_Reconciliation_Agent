...
2. For each file returned:
    ...
    c. Upload the structured JSON to the GCS bucket using `upload_json_to_gcs` tool by passing:
        - The JSON content EXACTLY as it was returned by `data_normalizer_agent`, without altering or reordering its keys.
...
- Preserve the exact key order in the JSON returned from `data_normalizer_agent`
"""